...
int hpx_main(hpx::program_options::variables_map& vm) {
  dlaf::initialize(vm);
  {
    ...
    std::tie(ref_a, ref_b, ref_x) =
        ::sampleLeftTr(uplo, op, diag, static_cast<T>(1.0) / alpha, ah.size().rows());
    ...
    for (int64_t run_index = -opts.nwarmups; run_index < opts.nruns; ++run_index) {
      ...
      dlaf::multiplication::triangular<Backend::Default, Device::Default, T>(comm_grid, side, uplo, op,
                                                                             diag, alpha, a.get(),
                                                                             b.get());
